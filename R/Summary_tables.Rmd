...
beta.params <- beta.params %>% arrange(match(Parameter, c("Intercept","DBH", "Lag-1", "Lag-2", "June Tmax", "MAP", "June Tmax x MAP")), desc(Cohort))
...
full.ghcn %>% filter(site %in% site.list) %>% filter(year %in% 1895:1950 & ageclass %in% "Past" | year %in% 1950:2015 & ageclass %in% "Modern") %>% group_by(site, ageclass) %>% 
  dplyr::summarise(
    Trees = length(unique(ID)),
...
for(i in 1:length(site.list)){
  ...
  df.corrs <- data.frame(Interseries.correlation = mean(interseries.cor(AVO)[,1]),
                         AR1 = mean(summary.AVO$ar1))
  write.csv(df.corrs, paste0("outputs/site_summary/", site.list[i], "_int_ar1.csv"), row.names = FALSE)
}


# get series intercorrelations for each site & cohort group
...
for(i in 1:length(site.list)){
  ...
  summary.AVO.mod <- summary.rwl(AVO.mod)
  
  df.corrs.mod <- data.frame(ageclass = "Modern",
                             
                             Interseries.correlation = ifelse(ncol(AVO.mod) > 2, mean(interseries.cor(AVO.mod)[,1]), NA ),
  ...
  AVO.past <- AVO.past %>% select(-year)
  summary.AVO.past <- summary.rwl(AVO.past)
  
  df.corrs.past <- data.frame(ageclass = "Past",
                              Interseries.correlation = ifelse(ncol(AVO.past) > 2, mean(interseries.cor(AVO.past)[,1]),NA ),
                         AR1 = mean(summary.AVO.past$ar1))
  df.corrs<- rbind(df.corrs.mod, df.corrs.past)
  
  write.csv(df.corrs, paste0("outputs/site_summary/", site.list[i], "_int_ar1_byageclass.csv"), row.names = FALSE)
}
