...
colnames(Yfut.35.35.samps) <- paste0(fut$site, "_",fut$year)
Yfut.35.35.samps$num <- 1:length(Yfut.35.35.samps$AVO_2015)
...
# make one big plot
ggplot(all.proj, aes(as.factor(year), RWI, fill = scenario))+geom_boxplot(position = "dodge2")

ggplot(all.proj, aes(as.factor(year), RWI, fill = scenario))+geom_boxplot(position = "dodge2")+facet_wrap(~site)
