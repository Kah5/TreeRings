...
rwi.ghcn$MAP.prism <- rowMeans(rwi.prism[,c("Month_pcp_1","Month_pcp_2", "Month_pcp_3","Month_pcp_4","Month_pcp_5","Month_pcp_6", "Month_pcp_7", "Month_pcp_8", "Month_pcp_9", "Month_pcp_10", "Month_pcp_11", "Month_pcp_12")])

rwi.ghcn$MAP.ghcn <- rowMeans(rwi.ghcn[,c("Month_pcp_1","Month_pcp_2", "Month_pcp_3","Month_pcp_4","Month_pcp_5","Month_pcp_6", "Month_pcp_7", "Month_pcp_8", "Month_pcp_9", "Month_pcp_10", "Month_pcp_11", "Month_pcp_12")])
...
ghcn.clean <- full.ghcn[!is.na(full.ghcn$RWI_1) & !is.na(full.ghcn$RWI_2),]
...
lag.model.by_structure <- jags.model(textConnection(population_model_structure_re_lag), 
                    data = list(Y=log(ghcn.clean$RWI), n=length(ghcn.clean$RWI), DI.scaled = ghcn.clean$DI.scaled, DBH.scaled = ghcn.clean$DBH.scaled, structure = as.numeric(ghcn.clean$structure),log_RWI_1 = log(ghcn.clean$RWI_1), SF = unique(ghcn.clean$structure), np=length(test$year), DI.scaled.p = test$DI.scaled, DBH.scaled.p = test$DBH.scaled, structure.p = as.numeric(test$structure), log_RWI_1.p = log(test$RWI_1)), n.chains = 3, n.adapt = 100)
...
mylabel = bquote(italic(R)^2 == .(format(pred.obs$r.squared, digits = 3)))

 png(width = 4, height = 6, units = "in", res = 300, "outputs/growth_model/lag_reg_structure_re/pred_vs_obs.png")
 plot(colMeans(exp(Yp.samps)), test$RWI, xlab = "Predicted Growth", ylab = "Observed Growth")
abline(a = 0, b = 1, col = "red")
text(x = 6, y = 2.5, labels = mylabel)
dev.off()

# now plot predicted vs. observed w/uncertainty:
...
ggplot(Yp.summary, aes(Observed, Predicted))+geom_point()+geom_pointrange(data = Yp.summary,aes(ymin=ci.lo, ymax=ci.hi))
...
lag.model.by_structure_x_cohort <- jags.model(textConnection(population_model_structure_x_cohort_re_lag), 
                    data = list(Y=log(ghcn.clean$RWI), n=length(ghcn.clean$RWI), DI.scaled = ghcn.clean$DI.scaled, DBH.scaled = ghcn.clean$DBH.scaled,log_RWI_1 = log(ghcn.clean$RWI_1), struct.cohort = as.numeric(ghcn.clean$struct.cohort.code), SF = unique(ghcn.clean$struct.cohort.code), np = length(test$DBH.scaled),
...
lag.model.by_structure_x_cohort <- jags.model(textConnection(site_model_structure_x_cohort_re_lag), 
                    data = list(Y=log(ghcn.clean$RWI), n=length(ghcn.clean$RWI), DI.scaled = ghcn.clean$DI.scaled, DBH.scaled = ghcn.clean$DBH.scaled,log_RWI_1 = log(ghcn.clean$RWI_1), struct.cohort = as.numeric(ghcn.clean$struct.cohort.code), SF = unique(ghcn.clean$struct.cohort.code), plots = unique(ghcn.clean$site), site = as.numeric(ghcn.clean$site), np = length(test$DBH.scaled),
...
lag2.model.by_structure_x_cohort <- jags.model(textConnection(population_model_structure_x_cohort_re_lag2), 
                    data = list(Y=log(ghcn.clean$RWI), n=length(ghcn.clean$RWI), DI.scaled = ghcn.clean$DI.scaled, DBH.scaled = ghcn.clean$DBH.scaled,log_RWI_1 = log(ghcn.clean$RWI_1),log_RWI_2 = log(ghcn.clean$RWI_2), struct.cohort = as.numeric(ghcn.clean$struct.cohort.code), SF = unique(ghcn.clean$struct.cohort.code), np = length(test$DBH.scaled),
...
lag2.model.by_structure_x_cohort <- jags.model(textConnection(model_structure_x_cohort_re_lag2), 
                    data = list(Y=ghcn.clean$RWI, n=length(ghcn.clean$RWI), DI.scaled = ghcn.clean$DI.scaled, DBH.scaled = ghcn.clean$DBH.scaled,log_RWI_1 = log(ghcn.clean$RWI_1),log_RWI_2 = ghcn.clean$RWI_2, struct.cohort = as.numeric(ghcn.clean$struct.cohort.code), SF = unique(ghcn.clean$struct.cohort.code), np = length(test$DBH.scaled),
...
reg.model.site_structure_x_cohort_sp6 <- jags.model(textConnection(site_model_structure_x_cohort_re_SP6_06 ), 
                    data = list(Y=log(train$RWI), n=length(train$RWI), DI.scaled = train$SP06_6, DBH.scaled = train$DBH.scaled, struct.cohort = as.numeric(train$struct.cohort.code), S = unique(train$struct.cohort.code), plot = as.numeric(train$site), ind = unique(train$site),np = length(test$DBH.scaled),
   struct.cohort.p = as.numeric(test$struct.cohort.code), DBH.scaled.p = test$DBH.scaled, DI.scaled.p = test$SP06_6, 
   nprobe = length(full.p$Var1),
   struct.cohort.probe = as.numeric(full.p$struct.cohort), DBH.scaled.probe = full.p$Var2, DI.scaled.probe = full.p$Var1
   ), n.chains = 3, n.adapt = 100)
...
# function g()
gfunc[i] <- beta1[struct.cohort[i]] + beta2[struct.cohort[i]]*DI.scaled[i] +  beta3[struct.cohort[i]]*DI.scaled2[i] + beta4[struct.cohort[i]]*DBH.scaled[i]   # use Drought index as a scaled variable 
...
# Prediction of test data
  for(i in 1:np){
    Yp[i]  ~ dnorm(mup[i],inv.var)
    mup[i] <- beta1[struct.cohort.p[i]] + beta2[struct.cohort.p[i]]*DI.scaled.p[i] + beta3[struct.cohort.p[i]]*DI.scaled2.p[i] + beta4[struct.cohort.p[i]]*DBH.scaled.p[i] 
  }
...
# Prediction of probe data for plotting:
 ...
 beta3[struct.cohort.probe[i]]*DI.scaled2.probe[i]+beta4[struct.cohort.probe[i]]*DBH.scaled.probe[i] 
  }
...
beta2[s] ~ dnorm(mu_beta2[plot[s]], inv_beta2)
beta3[s] ~ dnorm(mu_beta3[plot[s]], inv_beta3)
beta4[s] ~ dnorm(mu_beta4[plot[s]], inv_beta3)
}
...
unscale(vals = -1.4775, norm.data = DBH.scaled) # 10 cm
 ...
 DBH.p <- c(2.9355, 2.305,1.0441,0.414, -0.217, -0.8469, -1.4775 )# for 40 cm, 30 cm, 20cm, and 10 cm trees respectivly
 full.p <- expand.grid(DI.p,DI.p2, DBH.p,  struct.cohort= 1:4)
...
reg.model.site_structure_x_cohort_sp1_sp6 <- jags.model(textConnection(site_model_structure_x_cohort_re_SP6_01_06 ), 
                    data = list(Y=log(train$RWI), n=length(train$RWI), DI.scaled = train$SP01_6, DI.scaled2 = train$SP06_6, DBH.scaled = train$DBH.scaled, struct.cohort = as.numeric(train$struct.cohort.code), S = unique(train$struct.cohort.code), plot = as.numeric(train$site), ind = unique(train$site),np = length(test$DBH.scaled),
   struct.cohort.p = as.numeric(test$struct.cohort.code), DBH.scaled.p = test$DBH.scaled, DI.scaled.p = test$SP01_6, DI.scaled2.p = test$SP06_6,
   nprobe = length(full.p$Var1),
   struct.cohort.probe = as.numeric(full.p$struct.cohort), DBH.scaled.probe = full.p$Var3, DI.scaled.probe = full.p$Var1, DI.scaled2.probe = full.p$Var2
   ), n.chains = 3, n.adapt = 100)
...
#Extract the samples for each parameter for a basic exploration of effects
 ...
 test  <- readRDS(test, "outputs/growth_model/site_reg_structure_cohort_re_sp1_sp6/test.rds")
 ...
 sigma.samps <- samps[,(length(test$RWI)+length(full.p$Var1)+17)]
 sigma_betas <- samps[,(length(test$RWI)+length(full.p$Var1)+18):(length(test$RWI)+length(full.p$Var1)+20)]
...
abline(a = 0, b = 1, col = "red")
 dev.off()
# get predicted vs. observed R squared:
 pred.obs<- summary(lm(test$RWI~ colMeans(exp(Yp.samps))))
pred.obs$adj.r.squared
...
full.p$num <- 1:length(full.p[,1])
colnames(full.p) <- c("SP01","SP06", "DBH", "struct.cohort", "num")

# summarize by structure + cohort class only:
prob <- merge(probe.m, full.p, by = "num")
preds_ci <- prob %>% group_by(num, DBH,SP01, SP06, struct.cohort) %>% dplyr::summarise(meanY = mean(RWI),
                                                                            Ci.low = quantile(RWI, 0.025), Ci.high = quantile(RWI, 0.975))
...
# function g()
gfunc[i] <- beta1[struct.cohort[i]] + beta2[struct.cohort[i]]*DI.scaled[i] +  beta3[struct.cohort[i]]*DI.scaled2[i] + beta4[struct.cohort[i]]*DBH.scaled[i]+ beta5[struct.cohort[i]]*prevRWI.scaled[i]   # use Drought index as a scaled variable 
