...
gg.tile.past.mod <- ggplot(preds_ci_mod_past, aes(PDSI, DBH.class, fill = meanY))+geom_raster()+facet_wrap(~ageclass)+scale_fill_distiller(palette = "BrBG", direction = 1, name = "RWI")+ylab("Tree diameter")+xlab("Palmer Drought Severity Index")+theme_bw(base_size = 12)


png(height = 3.5, width = 8, units = "in", res = 300, "outputs/growth_model/site_reg_structure_cohort_re/predicted_growth_mod_past_tile.png")
gg.tile.past.mod
dev.off()


preds_ci_age_struct <- prob %>% group_by(Drought, struct.cohort) %>% summarise(meanY = mean(RWI),
                                                                            Ci.low = quantile(RWI, 0.025), Ci.high = quantile(RWI, 0.975))
...
ggplot(prob, aes(Drought, Ypred, color = as.factor(DBH)))+geom_point()+facet_wrap(~struct.cohort)
