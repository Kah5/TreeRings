...
lag2.model.by_structure_x_cohort <- jags.model(textConnection(population_model_structure_x_cohort_re_lag2), 
                    data = list(Y=log(train$RWI), n=length(train$RWI), DI.scaled = v$DI.scaled, DBH.scaled = train$DBH.scaled,log_RWI_1 = log(train$RWI_1),log_RWI_2 = log(train$RWI_2), struct.cohort = as.numeric(train$struct.cohort.code), SF = unique(train$struct.cohort.code), np = length(test$DBH.scaled),
...
samps <-  readRDS("outputs/growth_model/basic_reg_cohort_site_re/samps.rds")
 ...
 alpha.samps  <- samps[,(1:(length(test$RWI)+1)):(1:(length(test$RWI)+2))]
 beta2.samps <- samps[,(1:(length(test$RWI)+3)):(1:(length(test$RWI)+4))]
 beta3.samps <- samps[,(1:(length(test$RWI)+5)):(1:(length(test$RWI)+6))]
 mu_beta1.samps <- samps[,(1:(length(test$RWI)+7)):(1:(length(test$RWI)+10))]
 mu_beta2.samps <- samps[,(1:(length(test$RWI)+11)):(1:(length(test$RWI)+14))]
 mu_beta3.samps <- samps[,(1:(length(test$RWI)+15)):(1:(length(test$RWI)+18))]
 sigma.samps <- samps[,(1:(length(test$RWI)+19))]
 sigma_betas <- samps[,(1:(length(test$RWI)+20)):(1:(length(test$RWI)+22))]
...
model.summary <- data.frame(model = "basic_reg_cohort_site_re", 
           MSE = MSE1, 
           BIAS = BIAS1, 
           Rsq = pred.obs$r.squared)
