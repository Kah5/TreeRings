...
lag2.model.by_structure_x_cohort <- jags.model(textConnection(population_model_structure_x_cohort_re_lag2_temp), 
   ...
   struct.cohort.p = as.numeric(test$struct.cohort.code), DBH.scaled.p = test$DBH.scaled, DI.scaled.p = test$DI.scaled, log_RWI_1.p = log(test$RWI_1), log_RWI_2.p = log(test$RWI_2), Temp.scaled.p = test$T.scaled), n.chains = 3, n.adapt = 100)

update(lag2.model.by_structure_x_cohort, 1000); # Burnin for 1000 samples to start, then go higher later


samp.structure.cohort.re <- coda.samples(lag2.model.by_structure_x_cohort, 
                     variable.names=c("beta1", "beta2","beta3","beta4","beta5","beta6","sigma", "sigma_beta1", "sigma_beta2","sigma_beta3", "sigma_beta4","sigma_beta5", "sigma_beta6","Yp"), 
                    n.chains = 3, n.iter=20000, thin = 15)
...
#Extract the samples for each parameter for a basic exploration of effects
 ...
 beta4.samps <- samps[,(length(test$RWI)+ 13):(length(test$RWI)+ 16)]
 beta5.samps <- samps[,(length(test$RWI)+ 17):(length(test$RWI)+ 20)]
 beta6.samps <- samps[,(length(test$RWI)+ 21):(length(test$RWI)+ 24)]
 sigma.samps <- samps[,(length(test$RWI)+ 25)]
 sigma_betas <- samps[,(length(test$RWI)+ 26):(length(test$RWI)+ 31)]
