...
# FOR DRY YEARS ONLY: multi-level model with MAP as drought index stand structure by ageclass random effects and prev years growth (lag -1) and (lag - 2), and a site level random intercept parameter: 
...
# project into the future (assuming 35% further change in drought senseiivty):
  for(i in 1:nfut){
   Yfut.35[i]  ~ dnorm(mufut2[i],inv.var)
    mufut2[i] <- beta1[site.fut[i]] + (beta2[struct.cohort.fut[i]]*0.65)*DI.scaled.fut[i] + beta3[struct.cohort.fut[i]]*DBH.scaled.fut[i] +beta4[struct.cohort.fut[i]]*log_RWI_1.fut[i] +beta5[struct.cohort.fut[i]]*log_RWI_2.fut[i] + beta6[struct.cohort.fut[i]]*Temp.scaled.fut[i]
  }
...
# project into the future (assuming 50% further change in drought senseiivty):
  for(i in 1:nfut){
   Yfut.50[i]  ~ dnorm(mufut3[i],inv.var)
    mufut3[i] <- beta1[site.fut[i]] + (beta2[struct.cohort.fut[i]]*0.50)*DI.scaled.fut[i] + beta3[struct.cohort.fut[i]]*DBH.scaled.fut[i] +beta4[struct.cohort.fut[i]]*log_RWI_1.fut[i] +beta5[struct.cohort.fut[i]]*log_RWI_2.fut[i] + beta6[struct.cohort.fut[i]]*Temp.scaled.fut[i]
  }


# project into the future (assuming 35% further change in drought senseiivty & temperature):
  for(i in 1:nfut){
   Yfut.35.35[i]  ~ dnorm(mufut35[i],inv.var)
    mufut35[i] <- beta1[site.fut[i]] + (beta2[struct.cohort.fut[i]]*0.65)*DI.scaled.fut[i] + beta3[struct.cohort.fut[i]]*DBH.scaled.fut[i] +beta4[struct.cohort.fut[i]]*log_RWI_1.fut[i] +beta5[struct.cohort.fut[i]]*log_RWI_2.fut[i] + (beta6[struct.cohort.fut[i]]*1.35)*Temp.scaled.fut[i]
  }
...
# project into the future (assuming 50% further change in drought senseiivty):
  for(i in 1:nfut){
   Yfut.50.50[i]  ~ dnorm(mufut50[i],inv.var)
    mufut50[i] <- beta1[site.fut[i]] + (beta2[struct.cohort.fut[i]]*0.50)*DI.scaled.fut[i] + beta3[struct.cohort.fut[i]]*DBH.scaled.fut[i] +beta4[struct.cohort.fut[i]]*log_RWI_1.fut[i] +beta5[struct.cohort.fut[i]]*log_RWI_2.fut[i] + (beta6[struct.cohort.fut[i]]*1.50)*Temp.scaled.fut[i]
  }
...
train.clim.summary <- train.dry.pair[train.dry.pair$ageclass %in% "Modern",] %>% group_by(site) %>% summarise(
                                                MAP.scaled = mean(MAP.scaled), 
                                                 DBH.scaled = -0.413,
                                                T.scaled = mean(T.scaled),#, 
                                                RWI_1 = 0.103,
                                                RWI_2 = 0.103, struct.cohort.code = mean(struct.cohort.code2), 
                                                year = 2000)
...
fut <- rbind(future.probe_50, future.probe_70, train.clim.summary[,c("MAP.scaled", "DBH.scaled",  "T.scaled", "RWI_1", "RWI_2", "struct.cohort.code", "site","year")])
...
lag2.model.by_structure_x_cohort_t_pr <- jags.model(textConnection(population_model_site_int_structure_x_cohort_re_lag2_temp_MAP.f), 
  ...
  struct.cohort.p = as.numeric(test.dry.pair$struct.cohort.code), DBH.scaled.p = test.dry.pair$DBH.scaled, DI.scaled.p = test.dry.pair$DI.scaled, log_RWI_1.p = log(test.dry.pair$RWI_1), log_RWI_2.p = log(test.dry.pair$RWI_2), Temp.scaled.p = test.dry.pair$T.scaled, site.p = test.dry.pair$site.num, np = length(as.numeric(test.dry.pair$struct.cohort.code)), 
...
#samps <- samps[,(length(fut$RWI_1)+1):length(colnames(samps))]
 ...
 alpha.samps  <- samps[,(length(test.dry.pair$RWI)+1):(length(test.dry.pair$RWI)+ 9)]# one alpha for each of 4 cohort-strcuture groups
 beta2.samps <- samps[,(length(test.dry.pair$RWI)+10):(length(test.dry.pair$RWI)+13)]
 beta3.samps <- samps[,(length(test.dry.pair$RWI)+14):(length(test.dry.pair$RWI)+17)]
 beta4.samps <- samps[,(length(test.dry.pair$RWI)+18):(length(test.dry.pair$RWI)+ 21)]
 beta5.samps <- samps[,(length(test.dry.pair$RWI)+ 22):(length(test.dry.pair$RWI)+ 25)]
 beta6.samps <- samps[,(length(test.dry.pair$RWI)+ 26):(length(test.dry.pair$RWI)+ 29)]
 sigma.samps <- samps[,(length(test.dry.pair$RWI)+ 36)]
 sigma_betas <- samps[,(length(test.dry.pair$RWI)+ 30):(length(test.dry.pair$RWI)+ 35)]
...
png(height = 10, width = 4, units = "in",res = 200, "outputs/growth_model/lag2_reg_struct_x_cohort_re_t_pr_dry_yrs_site_rs/alpha_site_ints_vs_site_quality.png")
...
colnames(Yfut.samps) <- paste0(fut$site, "_",fut$year, "_", fut$DBH.scaled)
Yfut.samps$num <- 1:length(Yfut.samps$`AVO_2050_-0.413`)
fut.m <- melt(Yfut.samps)
fut.m$RWI <- exp(fut.m$value)
colnames(fut.m) <- c("site_year", "Ypred", "RWI")

fut$site_year <- paste0(fut$site, "_",fut$year, "_", fut$DBH.scaled)
fut$scenario <- "constant"
colnames(fut) <- c("site","MAP", "DBH","Tmax", "RWI_1","RWI_2","struct.cohort",  "year", "site.num","site_year", "scenario")
...
colnames(Yfut.35.samps) <-  paste0(fut$site, "_",fut$year, "_", fut$DBH.scaled)
...
colnames(Yfut.50.samps) <-  paste0(fut$site, "_",fut$year, "_", fut$DBH.scaled)
...
colnames(Yfut.35.35.samps) <-  paste0(fut$site, "_",fut$year, "_", fut$DBH.scaled)
...
colnames(Yfut.50.50.samps) <-  paste0(fut$site, "_",fut$year, "_", fut$DBH.scaled)
...
small.proj <- all.proj[all.proj$DBH == -0.413,]
...
# make one big plot
by.scenario <- ggplot(small.proj[!small.proj$year == 2000, ], aes(year, RWI, fill = as.character(year)))+geom_boxplot(position = "dodge2")+geom_hline(yintercept = mean(small.proj[small.proj$year ==2000,]$RWI, na.rm=TRUE), color = "black", linetype = "dashed")+facet_wrap(~scenario, nrow = 1, labeller = as_labeller(scenario_names))+ylab("Predicted tree growth (mm)")+xlab("Time period")+scale_fill_manual(values = c("blue", "red"))+theme(legend.title = element_blank())
...
by.period <- ggplot(small.proj[!small.proj$year == 2000, ], aes(year, RWI, fill = scenario))+geom_boxplot(position = "dodge2")+geom_hline(yintercept = mean(small.proj[small.proj$year ==2000 & small.proj$scenario %in% "constant",]$RWI, na.rm=TRUE), color = "black", linetype = "dashed")+ylab("Predicted tree growth (mm)")+xlab("Time period")+scale_fill_manual(values = c('#fee090','#91bfdb','#fc8d59','#4575b4','#d73027'
...
# project into the future with further %50 change in drought sensitivity:
  for(i in 1:nfut){
   Yfut[i]  ~ dnorm(mufut[i],inv.var)
    mufut[i] <- beta1[site.fut[i]] + beta2[struct.cohort.fut[i]]*DI.scaled.fut[i] + beta3[struct.cohort.fut[i]]*DBH.scaled.fut[i] +beta4[struct.cohort.fut[i]]*log_RWI_1.fut[i] +beta5[struct.cohort.fut[i]]*log_RWI_2.fut[i] + beta6[struct.cohort.fut[i]]*Temp.scaled.fut[i] + beta7[struct.cohort.fut[i]]*Temp.scaled.fut[i]*DI.scaled.fut[i]
  }
...
lag2.model.by_structure_x_cohort_t_pr_int <- jags.model(textConnection(population_model_site_int_structure_x_cohort_re_lag2_temp_MAP_interaction.f), 
  ...
  struct.cohort.fut = as.numeric(short.fut$struct.cohort.code), DBH.scaled.fut = short.fut$DBH.scaled, DI.scaled.fut = short.fut$MAP.scaled, log_RWI_1.fut = short.fut$RWI_1, log_RWI_2.fut =short.fut$RWI_2, Temp.scaled.fut = short.fut$T.scaled, site.fut = short.fut$site.num, nfut = length(as.numeric(short.fut$struct.cohort.code))
...
#Extract the samples for each parameter for a basic exploration of effects
 ...
 samps       <- samp.structure.cohort.re[[1]]
 saveRDS(samps, "outputs//growth_model/lag2_reg_struct_x_cohort_re_t_pr_dry_yrs_site/samps.rds")
 samps <- readRDS("outputs//growth_model/lag2_reg_struct_x_cohort_re_t_pr_dry_yrs_site/samps.rds")
...
#samps <- samps[,(length(fut$RWI_1)+1):length(colnames(samps))]
 ...
 sigma_betas <- samps[,34:40]
 sigma.samps <- samps[,41]
...
png(height = 3.5, width = 8, units = "in", res = 300,"outputs/growth_model/lag2_reg_struct_x_cohort_re_t_pr_dry_yrs_site_rs_inter/future_average_growth.png")
plot_grid(fut.year, fut.T, fut.map, align = "v", nrow = 1)
dev.off()
 

png(height = 5, width = 10, units = "in", res = 300,"outputs/growth_model/lag2_reg_struct_x_cohort_re_t_pr_dry_yrs_site_rs_inter/future_average_growth_by_site.png")
plot_grid(site.fut.tmax, sit.fut.map, ncol = 2)
dev.off()
```
