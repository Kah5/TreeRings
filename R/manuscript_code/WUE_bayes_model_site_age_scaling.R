...
full.iso <-readRDS( 'data/full_WUE_struct_cohort_scaled_dataset_v1sitescale.rds')
train.iso <- readRDS('data/train_WUE_struct_cohort_scaled_dataset_v1sitescale.rds')
test.iso <- readRDS('data/test_WUE_struct_cohort_scaled_dataset_v1sitescale.rds')
...
saveRDS(full.iso, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/full.iso_truct_cohort_scaled_dataset_v1sitescale.rds")
saveRDS(test.iso, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/test.iso_truct_cohort_scaled_dataset_v1sitescale.rds")
saveRDS(train.iso, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/train.iso_truct_cohort_scaled_dataset_v1sitescale.rds")
...
saveRDS(iWUE_map_tmax.re, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/samps.iWUE_struct.cohort.re_truct_cohort_scaled_dataset_v1sitescale.rds")
...
params <- samps[,1:16]
saveRDS(params, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/params.iWUE_struct.cohort_truct_cohort_scaled_dataset_v1sitescale.rds")
...
saveRDS(alpha.pct.diff, "outputs/growth_model/iWUE_MAP_TMAX_dbh/WUE_baseline_struct_cohort_differences_truct_cohort_scaled_dataset_v1sitescale.rds")
...
png(width = 6, height = 5, units = "in", res = 300, "outputs/growth_model/iWUE_MAP_TMAX_dbh/pred_vs_obs_struct_cohort_WUE_truct_cohort_scaled_dataset_v1sitescale.png")
p.o.plot
dev.off()
...
saveRDS(Predicted.growth, "outputs/growth_model/iWUE_MAP_TMAX_dbh/WUE_predicted_struct_cohort_truct_cohort_scaled_dataset_v1sitescale.rds")
...
#saveRDS(WUE.age, "outputs/growth_model/iWUE_MAP_TMAX_dbh/WUE_predicted_diffs_struct_cohort_truct_cohort_scaled_dataset_v1sitescale.rds")
...
saveRDS(iWUE_map_tmax.re, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/samps.iWUE_age.cohort.re_truct_cohort_scaled_dataset_v1sitescale.rds")
...
params <- samps[,1:8]
saveRDS(params, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/params.iWUE_ageclass_truct_cohort_scaled_dataset_v1sitescale.rds")
...
saveRDS(wue.pct.diff, "outputs/growth_model/iWUE_MAP_TMAX_dbh/WUE_baseline_cohort_differences_truct_cohort_scaled_dataset_v1sitescale.rds")
...
# note fairly poor model fit
png(width = 6, height = 5, units = "in", res = 300, "outputs/growth_model/iWUE_MAP_TMAX_dbh/pred_vs_obs_ageclass_WUE_truct_cohort_scaled_dataset_v1sitescale.png")
p.o.plot
dev.off()
...
saveRDS(Predicted.growth, "outputs/growth_model/iWUE_MAP_TMAX_dbh/WUE_predicted_cohort_truct_cohort_scaled_dataset_v1sitescale.rds")
...
# function g()
iWUEfunc[i] <- beta1[struct.cohort[i]] + beta2[struct.cohort[i]]*MAP_scaled[i] + beta3[struct.cohort[i]]*TMAX_scaled[i] + beta4[struct.cohort[i]]*DBH_scaled[i]#+beta5[tree[i]]
...
for(i in 1:np){
 ...
 # function g()
 iWUEfunc.p[i] <- beta1[struct.cohort.p[i]] + beta2[struct.cohort.p[i]]*MAP_scaled.p[i]+ beta3[struct.cohort.p[i]]*TMAX_scaled.p[i]+ beta4[struct.cohort.p[i]]*DBH_scaled.p[i]#+beta5[tree.p[i]]
...
id.crosswalk<- data.frame(ID.num = 1:length(unique(full.iso$ID)), 
           ID = unique(full.iso$ID) )

train.iso <- left_join( train.iso, id.crosswalk,by = "ID")
test.iso <- left_join( test.iso, id.crosswalk, by = "ID")
test.iso <- test.iso[!is.na(test.iso$struct.cohort.code),]
train.iso <- train.iso[!is.na(train.iso$struct.cohort.code),]
summary(lm(data = train.iso, formula = Cor.d13C.suess ~ T.scaled + MAP.scaled + DBH.scaled + DI.scaled + ID))
...
d13_map_tmax <- jags.model(textConnection(d13_MAP_tmax_dbh_re), 
                            data = list(iWUE =train.iso$Cor.d13C.suess, n=length(train.iso$Cor.d13C.suess), 
                                        ntree = length(unique(train.iso$ID)),
                                        tree = train.iso$ID.num,
                                        struct.cohort = as.numeric(train.iso$struct.cohort.code), 
                                        SF = unique(train.iso$struct.cohort.code),
                                        MAP_scaled = train.iso$MAP.scaled, TMAX_scaled = train.iso$T.scaled, 
                                        DBH_scaled = train.iso$DBH.scaled, site = train.iso$site.code,
                                        ...
                                        struct.cohort.p = as.numeric( test.iso$struct.cohort.code), 
                                        structure.p = as.numeric(test.iso$structure),
                                        tree.p = test.iso$ID.num,
...
saveRDS(d13C_map_tmax.re, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/samps.d13_struct.cohort_truct_cohort_scaled_dataset_v1sitescale.rds")
#Yp.samps <- samps[,1:660] # one alpha for each of 16 sites
#iWUEpred <- samps
...
saveRDS(params, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/params.d13_struct.cohort_truct_cohort_scaled_dataset_v1sitescale.rds")
...
# note fairly poor model fit
png(width = 6, height = 5, units = "in", res = 300, "outputs/growth_model/iWUE_MAP_TMAX_dbh/pred_vs_obs_d13C_truct_cohort_scaled_dataset_v1sitescale.png")
p.o.plot
dev.off()
...
d13samps       <- d13C_map_tmax.re [[1]]
saveRDS(d13samps, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/samps.d13_ageclass_truct_cohort_scaled_dataset_v1sitescale.rds")
#Yp.samps <- samps[,1:660] # one alpha for each of 16 sites
#iWUEpred <- samps
...
#beta4.samps  <- d13samps[,9:13]
params <- d13samps[,1:8]
saveRDS(params, "outputs/growth_model/iWUE_MAP_TMAX_dbh_cohort/params.d13_ageclass_truct_cohort_scaled_dataset_v1sitescale.rds")
...
# note fairly poor model fit
png(width = 6, height = 5, units = "in", res = 300, "outputs/growth_model/iWUE_MAP_TMAX_dbh/pred_vs_obs_d13C_ageclass_truct_cohort_scaled_dataset_v1sitescale.png")
p.o.plot
dev.off()
