...
wue.samps <- data.frame(wue.samps)
alpha.diff <- ((wue.samps[1] - wue.samps[2])/wue.samps[2])*100
...
### note that YP.full is not saved properly, and only represnts predictions of held out data

Yp.full <- readRDS( "outputs/growth_model/lag2_reg_cohort_only_re_t_pr_dry_yrs_site_rs_inter/Ypred_full_dataset.rds")
Yp.full <- as.data.frame(Yp.full[[1]])
YP.m <- melt(Yp.full)

# get average predicted growth for each tree in each year:
tree.yr <- YP.m %>% group_by(variable) %>% dplyr::summarise(mean.pred = mean(exp(value)),
                                                    ci.hi.p = quantile(exp(value),0.975),
                                                    ci.lo.p = quantile(exp(value),0.025))

# get the data used to predict the model
colsforYpred <- c("RWI", "MAP.scaled", "DBH.scaled", "RWI_1", "RWI_2", "T.scaled","ageclass", "ageclass_code", "site.num", "site")
full.dry.pair <- rbind(train.dry.pair[,colsforYpred], test.dry.pair[,colsforYpred])


tree.yr$RWI <- full.dry.pair$RWI
tree.yr$ageclass <- full.dry.pair$ageclass
tree.yr$ageclass <- factor(tree.yr$ageclass, levels = c("Past", "Modern"))
growth.cohort  <- tree.yr %>% group_by(ageclass) %>% dplyr::summarise(Predicted = mean(mean.pred),
                                                                      ci.hi = quantile(mean.pred,0.975),
                                                                      ci.lo = quantile(mean.pred,0.025), 
                                                                      Observed = mean(RWI))
...
png(height = 3, width = 12, units = "in", res = 300, "outputs/growth_model/paper_figures/summary_of_growth_mod.png")
...
png(height = 3, width = 12, units = "in", res = 300, "outputs/growth_model/paper_figures/summary_of_WUE_mod.png")
...
png(height = 9, width = 12, units = "in", res = 300, "outputs/growth_model/paper_figures/summary_of_d13_iwue_growth_mod.png")
plot_grid(mean.pred.d13.dot, mean.pred.beta1.d13, mean.pred.beta2.d13, mean.pred.beta3.d13,
...
png(height = 6, width = 12, units = "in", res = 300, "outputs/growth_model/paper_figures/summary_of_differences_cohort.png")
plot_grid(
