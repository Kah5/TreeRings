...
ggplot(data = load.loc, aes(coords.x1, coords.x2, fill= Comp.3))+ geom_jitter(width = 15000, height = 20000, size = 4,shape =21)+scale_fill_gradientn(colors =rev(brewer.pal(5,"Spectral")))+ geom_polygon(data=data.frame(mapdata), aes(x=long, y=lat, group=group),
                                                                                                                                                             colour = "darkgrey", fill = NA)+theme_linedraw()+ coord_cartesian(xlim = c(-59495.64, 724000), ylim=c(500000, 1500000))


#ggplot(data = load.loc, aes(coords.x1, coords.x2, color=Comp.3))+
ggplot(load.loc, aes(pr.av, sand, color=Comp.3))+geom_point(size = 4)+scale_color_gradientn(colors =rev(brewer.pal(5,"Spectral")))+theme_minimal()
ggplot(load.loc, aes(pr.av, sand, color=Comp.3))+geom_point(size = 4)+scale_color_gradientn(colors =rev(brewer.pal(5,"Spectral")))

d <- ggplot(load.loc, aes(coords.x1, coords.x2, color=Comp.4))+geom_point()

# basic plots of PC3 and 2 vs. climate
ggplot(load.loc,aes(pr.av, Comp.3))+geom_point()+geom_smooth(method = "lm")
